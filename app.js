...
async function loadAttendance(subjectId) {
  ...
  container.innerHTML = `<p class="muted">Loading attendance...</p>`;
  ...
  for (const record of data) {
    ...
    div.innerHTML = `
      ...
      <span class="rolls">Rolls: ${escapeHtml(rolls)}</span>
      <div class="actions">
...
$("saveAttendanceBtn").addEventListener("click", async () => {
  ...
  const rollsRaw = $("attendanceRolls").value.trim();
  ...
  if (!rollsRaw) {
    showMessage("attendanceMessage", "Please enter at least one roll number.", true);
    return;
  }

  // Parse roll numbers: split by comma, newline, or both
  const rolls = rollsRaw.split(/[\n,]+/).map(s => s.trim()).filter(s => s.length > 0);

  if (rolls.length === 0) {
    showMessage("attendanceMessage", "No valid roll numbers found.", true);
    return;
  }
  ...
  showMessage("attendanceMessage", "✅ Attendance saved successfully!");
  $("attendanceRolls").value = "";
